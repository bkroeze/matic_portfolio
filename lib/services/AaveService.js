...
class AaveService {
  ...
  getHistoricalRates(reserves, startDate, days, pool = "V2") {
    const daysList = range(0, days).map(delta => addDays(startDate, delta));
  ...
  getFormattedRates(startDate, days, pool="V2") {
    return this.getReserves()
        .then((reserves) => {
          return this.getHistoricalRates(reserves, startDate, days, pool);
       });
  }
