...
class AaveService {
  ...
  getHistoricalRates30DaysPriorToTime(startDate, pool = "V2") {
    return this.getHistoricalRatesAtTime(addDays(startDate, -30), pool);
  }
  ...
  getHistoricalRates(reserves, startDate, days, hours = false, pool = "V2") {
    ...
    const getRates = (day) => this.getHistoricalRates30DaysPriorToTime(day, pool);
    ...
    return Promise
      ...
      .then((dayRates) => {
        ...
        daysList.forEach((day, i) => {
          rates[day.toISOString()] = v2.formatReserves(reserves, getUnixTime(day), dayRates[i]);
        });
        return rates;
      });
  }
